...
double length_x = 1.0;//the size of the tissue (cm)*(cm)
double length_y = 1.0;
...
/* Voltage */
double V[nt + 1] = { 0 }; // Initial Voltage (mv)
double dV2[nt + 1] = { 0 }; // second order derivatives of Voltage (mv)
double Vnew[nt + 1] = { 0 };// New Voltage (mV)
...
double it[nt + 1] = { 0 };; // Total current (uA/cm^2)
int stim_i[nt + 1] = { 0 }; //{0,14,2,13,30,11,29,20,49,10,5, 3,48,6,9,8,27,16,50};// apply the stimulus on these elements 
int stim_size = 0;
...
double cai[nt + 1] = { 0 }; // Intracellular Ca Concentration (mM)
...
double ina[nt + 1] = { 0 }; // Fast Na Current (uA/uF)
...
double m[nt + 1] = { 0 }; // Na activation
double h[nt + 1] = { 0 }; // Na inactivation
double jj[nt + 1] = { 0 }; // Na slow inactivation

/* Current through L-type Ca Channel */
double isi[nt + 1] = { 0 }; // Slow inward current (uA/uF)
double esi[nt + 1] = { 0 }; // Reversal Potential of si (mV)
...
double d[nt + 1] = { 0 }; // Voltage dependant activation gate
double f[nt + 1] = { 0 }; // Voltage dependant inactivation gate
double fca[nt + 1] = { 0 }; // Ca dependant inactivation gate -from LR94
...
double ik[nt + 1] = { 0 }; // Rapidly Activating K Current (uA/uF)
double X[nt + 1] = { 0 }; // Rapidly Activating K time-dependant activation  --gate X in LR91
...
double ik1[nt + 1] = { 0 }; // Time-independent K current (uA/uF)
...
double ikp[nt + 1] = { 0 }; // Plateau K current (uA/uF)
...
/* Background Current */
double ib[nt + 1] = { 0 }; // Background current (uA/uF)
...
double v_norm(double v[2 + 1]);
double v_multiply(double v1[2 + 1], double v2[2 + 1]);
void v_subtract(double v[2 + 1], double v1[2 + 1], double v2[2 + 1]);
void v_assign(double v[2 + 1], double v1, double v2);
double m_det(double tmp[2 + 1][2 + 1]);

int main(int argc, char* argv[])
{
	int i;
	/******* Mesh processing *******/
	double nodes[np + 1][2 + 1] = { 0 };
	int elements[nt + 1][3 + 1] = { 0 };
	FILE *fp_nodes;
	...
	FILE *fp_elements;
	fp_elements = fopen("elements_c64.dat", "r");
	if (NULL == fp_elements)
		return 2;
	for (i = 1; i <= nt; i++)
	...
	int boundary_marker[nt + 1] = { 0 }; //whether a boundary triangle or not
	int numberOfneighbors[nt + 1] = { 0 };
	int adj_triangle[nt + 1][3 + 1] = { 0 }; // save the index of  three neighbor triangles
	double centroid[nt + 1][2 + 1] = { 0 }; //get the centroid coordinates
	double area[nt + 1] = { 0 }; // each triangle's area
	double Sf[nt + 1][3 + 1][2 + 1] = { 0 };//surface vector
	double gC[nt + 1][3 + 1] = { 0 }; //geometric interpolation factor related to the position of the element face f with respect to the nodes C and F.
	double weight[nt + 1][3 + 1] = { 0 }; //weighting factor
	double gradientU_C[nt + 1][2 + 1] = { 0 }; //the gradient at the centroid of an triangle with volume Vc
	double face_center[nt + 1][3 + 1][2 + 1] = { 0 }; //get the center coordinate of triangle's three faces
	double D_CF1[nt + 1][2 + 1] = { 0 }, D_CF2[nt + 1][2 + 1] = { 0 }, D_CF3[nt + 1][2 + 1] = { 0 };
	double ap[nt + 1] = { 0 };
	...
	for (i = 1; i <= nt; i++){
		...
		ap[i] = area[i] / dt;
		if (centroid[i][1] <= 0.04){// apply the stimulus on the elements if x coordinate <= 0.2 cm
			stim_size++;
			stim_i[stim_size] = i;
		}
		if ((centroid[i][2] <0.51 && centroid[i][2] >0.49) && centroid[i][1] <0.01){
			pos_L = i;
		}
		if ((centroid[i][2] <0.51 && centroid[i][2] >0.49) && centroid[i][1] >0.99){
			pos_R = i;
		}
	}
	...
	int p1, p2, p3;
	int tt;
	double diff_x, diff_y, Sf_tmp[2 + 1] = { 0 }, D_Cf[2 + 1] = { 0 };
	for (i = 1; i <= nt; i++){
		...
		for (tt = 1; tt <= nt; tt++){
			if (tt != i){
				...
				exist2 = 0;
				exist3 = 0;
				p1 = find_t(elements[i], 3, elements[tt][1]);
				p2 = find_t(elements[i], 3, elements[tt][2]);
				p3 = find_t(elements[i], 3, elements[tt][3]);
				if (p1 != 0){
					if (p1 == 1)
						exist1 = 1;
					else if (p1 == 2)
				...
				if (exist1 && exist2){
					adj_triangle[i][1] = tt;
					diff_x = nodes[elements[i][1]][1] - nodes[elements[i][2]][1];
					diff_y = nodes[elements[i][1]][2] - nodes[elements[i][2]][2];
					face_center[i][1][1] = 0.5*(nodes[elements[i][1]][1] + nodes[elements[i][2]][1]);
					face_center[i][1][2] = 0.5*(nodes[elements[i][1]][2] + nodes[elements[i][2]][2]);
					v_assign(Sf_tmp, diff_y, -diff_x);
					Sf[i][1][1] = diff_y;
					Sf[i][1][2] = -diff_x;
					v_assign(D_Cf, face_center[i][1][1] - centroid[i][1], face_center[i][1][2] - centroid[i][2]);
					v_assign(D_CF1[i], centroid[tt][1] - centroid[i][1], centroid[tt][2] - centroid[i][2]);
					gC[i][1] = weight_cal(centroid[i][1], centroid[i][2], centroid[tt][1], centroid[tt][2],
						nodes[elements[i][1]][1], nodes[elements[i][1]][2], nodes[elements[i][2]][1], nodes[elements[i][2]][2]);//0.5;
					weight[i][1] = 1 / pow(sqrt(D_CF1[i][1] * D_CF1[i][1] + D_CF1[i][2] * D_CF1[i][2]), 3);
					if ((D_Cf[1] * Sf_tmp[1] + D_Cf[2] * Sf_tmp[2]) < 1){
						Sf[i][1][1] = -diff_y;
						Sf[i][1][2] = diff_x;
					}
					flag1 = 1;
					...
					adj_triangle[i][2] = tt;
					diff_x = nodes[elements[i][1]][1] - nodes[elements[i][3]][1];
					diff_y = nodes[elements[i][1]][2] - nodes[elements[i][3]][2];
					face_center[i][2][1] = 0.5*(nodes[elements[i][1]][1] + nodes[elements[i][3]][1]);
					face_center[i][2][2] = 0.5*(nodes[elements[i][1]][2] + nodes[elements[i][3]][2]);
					v_assign(Sf_tmp, diff_y, -diff_x);
					Sf[i][2][1] = diff_y;
					Sf[i][2][2] = -diff_x;
					v_assign(D_Cf, face_center[i][2][1] - centroid[i][1], face_center[i][2][2] - centroid[i][2]);
					v_assign(D_CF2[i], centroid[tt][1] - centroid[i][1], centroid[tt][2] - centroid[i][2]);
					gC[i][2] = weight_cal(centroid[i][1], centroid[i][2], centroid[tt][1], centroid[tt][2],
						nodes[elements[i][1]][1], nodes[elements[i][1]][2], nodes[elements[i][3]][1], nodes[elements[i][3]][2]);//0.5;
					weight[i][2] = 1 / pow(sqrt(D_CF2[i][1] * D_CF2[i][1] + D_CF2[i][2] * D_CF2[i][2]), 3);
					if ((D_Cf[1] * Sf_tmp[1] + D_Cf[2] * Sf_tmp[2]) < 0){
						Sf[i][2][1] = -diff_y;
						Sf[i][2][2] = diff_x;
					}
					flag2 = 1;
					...
					adj_triangle[i][3] = tt;
					diff_x = nodes[elements[i][2]][1] - nodes[elements[i][3]][1];
					diff_y = nodes[elements[i][2]][2] - nodes[elements[i][3]][2];
					face_center[i][3][1] = 0.5*(nodes[elements[i][2]][1] + nodes[elements[i][3]][1]);
					face_center[i][3][2] = 0.5*(nodes[elements[i][2]][2] + nodes[elements[i][3]][2]);
					v_assign(Sf_tmp, diff_y, -diff_x);
					Sf[i][3][1] = diff_y;
					Sf[i][3][2] = -diff_x;
					v_assign(D_Cf, face_center[i][3][1] - centroid[i][1], face_center[i][3][2] - centroid[i][2]);
					v_assign(D_CF3[i], centroid[tt][1] - centroid[i][1], centroid[tt][2] - centroid[i][2]);
					gC[i][3] = weight_cal(centroid[i][1], centroid[i][2], centroid[tt][1], centroid[tt][2],
						nodes[elements[i][2]][1], nodes[elements[i][2]][2], nodes[elements[i][3]][1], nodes[elements[i][3]][2]);//0.5;
					weight[i][3] = 1 / pow(sqrt(D_CF3[i][1] * D_CF3[i][1] + D_CF3[i][2] * D_CF3[i][2]), 3);
					if ((D_Cf[1] * Sf_tmp[1] + D_Cf[2] * Sf_tmp[2]) < 0){
						Sf[i][3][1] = -diff_y;
						Sf[i][3][2] = diff_x;
					}
		...
		if (adj_triangle[i][1] == 0 || adj_triangle[i][2] == 0 || adj_triangle[i][3] == 0)
			boundary_marker[i] = 1; //whether a boundary triangle or not
	}

	double aC[nt + 1] = { 0 };
	double aF[nt + 1][3 + 1] = { 0 };
	double bC[nt + 1][3 + 1] = { 0 }; //non - orthogonal flux
	double Tf[nt + 1][3 + 1][2 + 1] = { 0 }; // component Tf is normal to Sf
	double matrix_G[nt + 1][2 + 1][2 + 1] = { 0 }; //for Least - Square method in Section 9.3
	int Gauss[nt + 1] = { 0 }; //triangles needed to use Green - Gauss method
	int count = 1;
	double unit_CF[nt + 1][3 + 1][2 + 1] = { 0 }, Ef[2 + 1] = { 0 };
	double tmp[2 + 1][2 + 1] = { 0 };
	double Ef_value[3 + 1] = { 0 };
	...
	for (i = 1; i <= nt; i++){
		...
		if (adj_triangle[i][1] != 0){ // if not boundary face
			v_assign(unit_CF[i][1], D_CF1[i][1] / v_norm(D_CF1[i]), D_CF1[i][2] / v_norm(D_CF1[i]));//the unit vector e in the direction of CF
			v_assign(Sf_tmp, Sf[i][1][1], Sf[i][1][2]);
			v_assign(Ef, (v_multiply(Sf_tmp, Sf_tmp) / v_multiply(unit_CF[i][1], Sf_tmp))*unit_CF[i][1][1],//over - relaxed approach in Section 8.6.4
				(v_multiply(Sf_tmp, Sf_tmp) / v_multiply(unit_CF[i][1], Sf_tmp))*unit_CF[i][1][2]);
			Ef_value[1] = v_norm(Ef);
			v_subtract(Tf[i][1], Sf_tmp, Ef);//decomposition, Sf = Tf + Ef;
			Distance_CF1 = v_norm(D_CF1[i]);

			matrix_G[i][1][1] = matrix_G[i][1][1] + weight[i][1] * D_CF1[i][1] * D_CF1[i][1];
			matrix_G[i][1][2] = matrix_G[i][1][2] + weight[i][1] * D_CF1[i][1] * D_CF1[i][2];
			matrix_G[i][2][1] = matrix_G[i][2][1] + weight[i][1] * D_CF1[i][1] * D_CF1[i][2];
			matrix_G[i][2][2] = matrix_G[i][2][2] + weight[i][1] * D_CF1[i][2] * D_CF1[i][2];
		}
		if (adj_triangle[i][2] != 0){ // if not boundary face
			v_assign(unit_CF[i][2], D_CF2[i][1] / v_norm(D_CF2[i]), D_CF2[i][2] / v_norm(D_CF2[i]));
			v_assign(Sf_tmp, Sf[i][2][1], Sf[i][2][2]);
			v_assign(Ef, (v_multiply(Sf_tmp, Sf_tmp) / v_multiply(unit_CF[i][2], Sf_tmp))*unit_CF[i][2][1],
				(v_multiply(Sf_tmp, Sf_tmp) / v_multiply(unit_CF[i][2], Sf_tmp))*unit_CF[i][2][2]);
			Ef_value[2] = v_norm(Ef);
			v_subtract(Tf[i][2], Sf_tmp, Ef);//decomposition, Sf = Tf + Ef;
			Distance_CF2 = v_norm(D_CF2[i]);

			matrix_G[i][1][1] = matrix_G[i][1][1] + weight[i][2] * D_CF2[i][1] * D_CF2[i][1];
			matrix_G[i][1][2] = matrix_G[i][1][2] + weight[i][2] * D_CF2[i][1] * D_CF2[i][2];
			matrix_G[i][2][1] = matrix_G[i][2][1] + weight[i][2] * D_CF2[i][1] * D_CF2[i][2];
			matrix_G[i][2][2] = matrix_G[i][2][2] + weight[i][2] * D_CF2[i][2] * D_CF2[i][2];
		}
		if (adj_triangle[i][3] != 0){ // if not boundary face
			v_assign(unit_CF[i][3], D_CF3[i][1] / v_norm(D_CF3[i]), D_CF3[i][2] / v_norm(D_CF3[i]));
			v_assign(Sf_tmp, Sf[i][3][1], Sf[i][3][2]);
			v_assign(Ef, (v_multiply(Sf_tmp, Sf_tmp) / v_multiply(unit_CF[i][3], Sf_tmp))*unit_CF[i][3][1],
				(v_multiply(Sf_tmp, Sf_tmp) / v_multiply(unit_CF[i][3], Sf_tmp))*unit_CF[i][3][2]);
			Ef_value[3] = v_norm(Ef);
			v_subtract(Tf[i][3], Sf_tmp, Ef);//decomposition, Sf = Tf + Ef;
			Distance_CF3 = v_norm(D_CF3[i]);

			matrix_G[i][1][1] = matrix_G[i][1][1] + weight[i][3] * D_CF3[i][1] * D_CF3[i][1];
			matrix_G[i][1][2] = matrix_G[i][1][2] + weight[i][3] * D_CF3[i][1] * D_CF3[i][2];
			matrix_G[i][2][1] = matrix_G[i][2][1] + weight[i][3] * D_CF3[i][1] * D_CF3[i][2];
			matrix_G[i][2][2] = matrix_G[i][2][2] + weight[i][3] * D_CF3[i][2] * D_CF3[i][2];
		}

		//aC(i) = Ef_value(i, 1) / Distance_CF1 + Ef_value(i, 2) / Distance_CF2 + Ef_value(i, 3) / Distance_CF3;
		if (Distance_CF1 != 0)
			aF[i][1] = D * Ef_value[1] / Distance_CF1;
		if (Distance_CF2 != 0)
			aF[i][2] = D *  Ef_value[2] / Distance_CF2;
		if (Distance_CF3 != 0)
			aF[i][3] = D *  Ef_value[3] / Distance_CF3;
		aC[i] = aF[i][1] + aF[i][2] + aF[i][3];

		//decide whether singular matrix or not
		tmp[1][1] = matrix_G[i][1][1];
		tmp[1][2] = matrix_G[i][1][2];
		tmp[2][1] = matrix_G[i][2][1];
		tmp[2][2] = matrix_G[i][2][2];
	...
	double Uf[nt + 1][3 + 1] = { 0 };//function value at the triangle face center
	for (i = 1; i <= nt; i++){
	...
	//----------Big loop for time t--------------------------------------
	double vector_h[2 + 1] = { 0 }, gradient_f1[2 + 1] = { 0 }, gradient_f1_tmp[2 + 1] = { 0 },
		gradient_f2[2 + 1] = { 0 }, gradient_f2_tmp[2 + 1] = { 0 }, gradient_f3[2 + 1] = { 0 },
		gradient_f3_tmp[2 + 1] = { 0 };
	double sum_t;
...
#pragma omp parallel private(ncount,tid)// create and destruction of threads takes time, so it's a good place to create all threads here!!
	{
		...
		for (ncount = 1; ncount <= 500 / dt; ncount++){//simulation time is 500ms
...
#pragma omp for private(i,vector_h,tmp) schedule(static)
			for (i = 1; i <= nt; i++){
				memset(vector_h, 0, sizeof(vector_h));
				if (adj_triangle[i][1] != 0){
					vector_h[1] = vector_h[1] + weight[i][1] * D_CF1[i][1] * (V[adj_triangle[i][1]] - V[i]);
					vector_h[2] = vector_h[2] + weight[i][1] * D_CF1[i][2] * (V[adj_triangle[i][1]] - V[i]);
				}
				if (adj_triangle[i][2] != 0){
					vector_h[1] = vector_h[1] + weight[i][2] * D_CF2[i][1] * (V[adj_triangle[i][2]] - V[i]);
					vector_h[2] = vector_h[2] + weight[i][2] * D_CF2[i][2] * (V[adj_triangle[i][2]] - V[i]);
				}
				if (adj_triangle[i][3] != 0){
					vector_h[1] = vector_h[1] + weight[i][3] * D_CF3[i][1] * (V[adj_triangle[i][3]] - V[i]);
					vector_h[2] = vector_h[2] + weight[i][3] * D_CF3[i][2] * (V[adj_triangle[i][3]] - V[i]);
				}
				tmp[1][1] = matrix_G[i][1][1];
				tmp[1][2] = matrix_G[i][1][2];
				tmp[2][1] = matrix_G[i][2][1];
				...
				if (fabs(m_det(tmp)) > 1e-10){//singular matrix cannot be computed
					...
					if (tmp[1][1] != 0){// solve Ax=b
						gradientU_C[i][2] = (vector_h[2] - (tmp[2][1] * vector_h[1]) / tmp[1][1]) / (tmp[2][2] - (tmp[2][1] * tmp[1][2]) / tmp[1][1]);
						gradientU_C[i][1] = (vector_h[1] - tmp[1][2] * gradientU_C[i][2]) / tmp[1][1];
					}
					else{
						gradientU_C[i][2] = vector_h[1] / tmp[1][2];
						gradientU_C[i][1] = (vector_h[2] - tmp[2][2] * gradientU_C[i][2]) / tmp[2][1];
					}
...
#pragma omp for private(i) schedule(static)
			for (i = 1; i <= nt; i++){
				if (find_t(Gauss, nt, i)){
					if (adj_triangle[i][1] != 0)
						Uf[i][1] = (1 - gC[i][1])*V[adj_triangle[i][1]] + gC[i][1] * V[i];
					else
						Uf[i][1] = V[i];
					if (adj_triangle[i][2] != 0)
						Uf[i][2] = (1 - gC[i][2])*V[adj_triangle[i][2]] + gC[i][2] * V[i];
					else
						Uf[i][2] = V[i];
					if (adj_triangle[i][3] != 0)
						Uf[i][3] = (1 - gC[i][3])*V[adj_triangle[i][3]] + gC[i][3] * V[i];
					else
						Uf[i][3] = V[i];

					gradientU_C[i][1] = (1.0 / area[i])*(Uf[i][1] * Sf[i][1][1] + Uf[i][2] * Sf[i][2][1] + Uf[i][3] * Sf[i][3][1]);
					gradientU_C[i][2] = (1.0 / area[i])*(Uf[i][1] * Sf[i][1][2] + Uf[i][2] * Sf[i][2][2] + Uf[i][3] * Sf[i][3][2]);
...
#pragma omp for private(i,gradient_f1_tmp,gradient_f1,gradient_f2_tmp,gradient_f2,gradient_f3_tmp,gradient_f3) schedule(static)
			for (i = 1; i <= nt; i++){
				if (adj_triangle[i][1] != 0){
					gradient_f1_tmp[1] = (1 - gC[i][1])*gradientU_C[adj_triangle[i][1]][1] + gC[i][1] * gradientU_C[i][1];
					gradient_f1_tmp[2] = (1 - gC[i][1])*gradientU_C[adj_triangle[i][1]][2] + gC[i][1] * gradientU_C[i][2];
					//Note: here a tmp variable "gradient_f1_tmp" is needed to avoid across assignment
					gradient_f1[1] = gradient_f1_tmp[1] + ((V[adj_triangle[i][1]] - V[i]) / v_norm(D_CF1[i]) - v_multiply(gradient_f1_tmp, unit_CF[i][1]))*unit_CF[i][1][1];
					gradient_f1[2] = gradient_f1_tmp[2] + ((V[adj_triangle[i][1]] - V[i]) / v_norm(D_CF1[i]) - v_multiply(gradient_f1_tmp, unit_CF[i][1]))*unit_CF[i][1][2];
				}
				else{
					gradient_f1[1] = 0; //boundary edge
					gradient_f1[2] = 0;
				}

				if (adj_triangle[i][2] != 0){
					gradient_f2_tmp[1] = (1 - gC[i][2])*gradientU_C[adj_triangle[i][2]][1] + gC[i][2] * gradientU_C[i][1];
					gradient_f2_tmp[2] = (1 - gC[i][2])*gradientU_C[adj_triangle[i][2]][2] + gC[i][2] * gradientU_C[i][2];
					gradient_f2[1] = gradient_f2_tmp[1] + ((V[adj_triangle[i][2]] - V[i]) / v_norm(D_CF2[i]) - v_multiply(gradient_f2_tmp, unit_CF[i][2]))*unit_CF[i][2][1];
					gradient_f2[2] = gradient_f2_tmp[2] + ((V[adj_triangle[i][2]] - V[i]) / v_norm(D_CF2[i]) - v_multiply(gradient_f2_tmp, unit_CF[i][2]))*unit_CF[i][2][2];
				}
				else{
					gradient_f2[1] = 0; //boundary edge
					gradient_f2[2] = 0;
				}

				if (adj_triangle[i][3] != 0){
					gradient_f3_tmp[1] = (1 - gC[i][3])*gradientU_C[adj_triangle[i][3]][1] + gC[i][3] * gradientU_C[i][1];
					gradient_f3_tmp[2] = (1 - gC[i][3])*gradientU_C[adj_triangle[i][3]][2] + gC[i][3] * gradientU_C[i][2];
					gradient_f3[1] = gradient_f3_tmp[1] + ((V[adj_triangle[i][3]] - V[i]) / v_norm(D_CF3[i]) - v_multiply(gradient_f3_tmp, unit_CF[i][3]))*unit_CF[i][3][1];
					gradient_f3[2] = gradient_f3_tmp[2] + ((V[adj_triangle[i][3]] - V[i]) / v_norm(D_CF3[i]) - v_multiply(gradient_f3_tmp, unit_CF[i][3]))*unit_CF[i][3][2];
				}
				else{
					gradient_f3[1] = 0; //boundary edge
					gradient_f3[2] = 0;
				}
				bC[i][1] = D * v_multiply(gradient_f1, Tf[i][1]); //non-orthogonal flux
				bC[i][2] = D * v_multiply(gradient_f2, Tf[i][2]);
				bC[i][3] = D * v_multiply(gradient_f3, Tf[i][3]);
			}
#pragma omp barrier

			//flux = 0;
#pragma omp for private(i,sum_t) schedule(static)
			for (i = 1; i <= nt; i++){
				sum_t = 0;
				//double aa1, aa2, aa3;
				if (adj_triangle[i][1] != 0){
					sum_t = sum_t + aF[i][1] * V[adj_triangle[i][1]];
					//	aa1 = aF[i][1] * (V[adj_triangle[i][1]] - V[i]);
				}
				if (adj_triangle[i][2] != 0){
					sum_t = sum_t + aF[i][2] * V[adj_triangle[i][2]];
					//	aa2 = aF[i][2] * (V[adj_triangle[i][2]] - V[i]);
				}
				if (adj_triangle[i][3] != 0){
					sum_t = sum_t + aF[i][3] * V[adj_triangle[i][3]];
					//	aa3 = aF[i][3] * (V[adj_triangle[i][3]] - V[i]);
				}
				//Vnew[i] = (sum_t + (ap[i] - (aF[i][1] + aF[i][2] + aF[i][3]))*V[i] + bC[i][1] + bC[i][2] + bC[i][3]) / ap[i];
				Vnew[i] = V[i] + (sum_t + (-(aF[i][1] + aF[i][2] + aF[i][3]))*V[i] + bC[i][1] + bC[i][2] + bC[i][3]) / ap[i];
